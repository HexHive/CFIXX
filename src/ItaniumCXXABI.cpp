...
llvm::Value *ItaniumCXXABI::getVirtualFunctionPointer(CodeGenFunction &CGF,
  ...
  /*NHB CFIXX modifications - use our vtable instead of the one in the object*/
  //llvm::Value *VTable = CGF.GetVTablePtr(This, Ty, MethodDecl->getParent());
  llvm::Value *VTable = CGF.GetVTablePtrCFIXX(This, Ty, MethodDecl->getParent());
...
#include "clang/AST/TypeNodes.def"
  ...
  /*NHB CFIXX - Deal with RTTI objects which are created at compile time*/
//#ifdef CFIXX_FOO
  std::string cfixxName = "__cfixx_RTTI_metadata_init_";
  cfixxName.append(CGM.getModule().getName());
	for(unsigned long i = 0; i < cfixxName.length(); i++) {
			/* Only alphanumeric and underscores are allowed as function names */
			if(!((cfixxName[i] >= 'a' && cfixxName[i] <= 'z') ||
									(cfixxName[i] >= 'A' && cfixxName[i] <= 'Z') ||
									(cfixxName[i] >= '0' && cfixxName[i] <= '9') ||
									(cfixxName[i] == '_')) ) { 
					cfixxName[i] = '_';
			}
	}
  ...
  //llvm::errs() << cfixxName << "\n";
  llvm::FunctionType *ft = llvm::FunctionType::get(CGM.VoidTy, false);
  llvm::Function *cfixxInit = CGM.getModule().getFunction(cfixxName);
  llvm::BasicBlock *bb;
  if(!cfixxInit) {
    //Create the function, add a basic block, return, and schedule it to run
    cfixxInit = llvm::Function::Create(ft,
        llvm::GlobalValue::LinkageTypes::ExternalLinkage,
        cfixxName,
        &(CGM.getModule()));
    assert(cfixxInit && "Failed to create initializer for RTTI");
    bb = llvm::BasicBlock::Create(CGM.getModule().getContext(), "", cfixxInit, nullptr);
    llvm::ReturnInst::Create(CGM.getModule().getContext(), bb);
    CGM.AddGlobalCtor(cfixxInit, 0);
    //llvm::appendToGlobalCtors(CGM.getModule(), cfixxInit, 0);
  }
  else 
    bb = &(cfixxInit->getEntryBlock());
  llvm::IRBuilder<> IRB(&(bb->back()));
  
  llvm::Constant *cfixxSetVTablePtr = CGM.getModule().getOrInsertFunction("cfixxSetVTablePtr", CGM.VoidTy, CGM.VoidPtrTy, CGM.VoidPtrTy, NULL);
  llvm::Value *vtableptr = IRB.CreateStructGEP(Init->getType(), GV, 0);
  vtableptr = IRB.CreateLoad(vtableptr);
  vtableptr = IRB.CreateBitCast(vtableptr, CGM.Int8PtrTy);
  llvm::Value *loc = IRB.CreateBitCast(GV, CGM.Int8PtrTy);
  IRB.CreateCall(cfixxSetVTablePtr, {loc, vtableptr});
  //cfixxInit->dump();
  /*End NHB CFIXX modifications*/
//#endif
