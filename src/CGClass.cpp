...
void CodeGenFunction::EmitDestructorBody(FunctionArgList &Args) {
  ...
  /*NHB CFIXX modifications -- insert call to our cfixxDtor function*/
  /*Now doing this on Delete / out of scope */
  /*
  llvm::FunctionType *FTy =
      llvm::FunctionType::get(CGM.VoidTy, CGM.VoidPtrTy, false);
  llvm::Constant *F = CGM.CreateRuntimeFunction(FTy, "cfixxDtor");
  //TODO: Switch to VTableLocation pointer instead
  llvm::Value *thisPtr = Builder.CreateBitCast(CXXThisValue, CGM.VoidPtrTy);
  Builder.CreateCall(F, {thisPtr});
  */
  /*NHB CFIXX end modifications*/
...
void CodeGenFunction::InitializeVTablePointer(const VPtr &Vptr) {
  ...
  /*NHB CFIXX modification -- update our metadata*/
  //Deals with indexing, and determining if the second level table is allocated
  //llvm::Module &M = CGM.getModule();
  //llvm::Value *cfixxLookupGlobal = M.getOrInsertGlobal("cfixxLookup", CGM.VoidPtrTy->getPointerTo());
  llvm::Constant *cfixxLookupInt = llvm::ConstantInt::get(CGM.Int64Ty, 0x7f0104846000UL);
  llvm::Constant *cfixxLookupPtr = llvm::ConstantExpr::getIntToPtr(cfixxLookupInt, CGM.VoidPtrTy->getPointerTo()->getPointerTo());
//  llvm::Value *cfixxNextSecondLevelGlobal = M.getOrInsertGlobal("cfixxNextSecondLevel", CGM.VoidPtrTy->getPointerTo());
//  llvm::Value *cfixxTableEndGlobal = M.getOrInsertGlobal("cfixxTableEnd", CGM.VoidPtrTy->getPointerTo());
  //llvm::Value *cfixxColdGlobal = M.getOrInsertGlobal("cfixxCold", CGM.VoidPtrTy->getPointerTo());
  llvm::Constant *cfixxColdInt = llvm::ConstantInt::get(CGM.Int64Ty, 0x7f53feacb000UL);
  llvm::Constant *cfixxColdPtr = llvm::ConstantExpr::getIntToPtr(cfixxColdInt, CGM.VoidPtrTy->getPointerTo());
  ...
  //load pointer to second level from first level
  //Address cfixxLookupPtr = Address(cfixxLookupGlobal, getPointerAlign());
  //llvm::Value *cfixxLookup = Builder.CreateLoad(cfixxLookupPtr);
  tmp = Builder.CreateGEP(cfixxLookupPtr, tmp);
  //tmp = Builder.CreateBitCast(tmp, CGM.VoidPtrTy->getPointerTo()->getPointerTo());
  Address secondLevelPtr = Address(tmp, getPointerAlign()); 
  llvm::Instruction *secondLevel = Builder.CreateLoad(secondLevelPtr);
  
  //second level index
  tmp = Builder.CreateLShr(thisPtr, 3);
  llvm::Value *level2Idx = Builder.CreateAnd(tmp, 8388607);
  
  //The comparison to determine if we are on the hot or cold path
  //Address cfixxColdPtr = Address(cfixxColdGlobal, getPointerAlign());
  //llvm::Value *cfixxCold = Builder.CreateLoad(cfixxColdPtr);
  tmp = Builder.CreateICmpEQ(secondLevel, cfixxColdPtr);
  
  //branch to hot path or cold path
  llvm::BasicBlock *coldPath = createBasicBlock("coldPath");
  ...
  EmitBlock(coldPath);
  llvm::FunctionType *FTy =
      llvm::FunctionType::get(CGM.VoidPtrTy, CGM.Int64Ty, false);
  ...
  /*Hot Path Code*/
  EmitBlock(hotPath);
  llvm::PHINode *phi = Builder.CreatePHI(CGM.VoidPtrTy->getPointerTo(), 2, "cfixxStoreLoc");
  phi->addIncoming(secondLevel, secondLevel->getParent()); 
  phi->addIncoming(slowReturn, coldPath); 
  tmp = Builder.CreateGEP(phi, level2Idx);
  //Builder.CreateStore(vtablePtr, Address(tmp, getPointerAlign()));
  ///*
  FTy = llvm::FunctionType::get(CGM.VoidTy, {CGM.VoidPtrTy, CGM.VoidPtrTy->getPointerTo()}, false);
...
/*NHB CFIXX Addition - look up our VTablePtr from metadata*/
llvm::Value *CodeGenFunction::GetVTablePtrCFIXX(Address This,
                                           llvm::Type *VTableTy,
                                           const CXXRecordDecl *RD) {
  /*Inline cfixxVTable starting here*/
  //llvm::Module &M = CGM.getModule();
  llvm::Constant *cfixxLookupInt = llvm::ConstantInt::get(CGM.Int64Ty, 0x7f0104846000UL);
  //llvm::Constant *cfixxLookupIntCasted = llvm::ConstantExpr::getBitCast(cfixxLookupInt, CGM.VoidPtrTy->getPointerTo()->getPointerTo());
  llvm::Constant *cfixxLookupPtr = llvm::ConstantExpr::getIntToPtr(cfixxLookupInt, CGM.VoidPtrTy->getPointerTo()->getPointerTo());
  //llvm::Value *cfixxLookupGlobal = M.getOrInsertGlobal("cfixxLookup", CGM.VoidPtrTy->getPointerTo());
  
  //Pointer we are looking up vtable for
  llvm::Value *thisPtr = Builder.CreateBitCast(This.getPointer(), CGM.VoidPtrTy);
  thisPtr = Builder.CreatePtrToInt(This.getPointer(), CGM.Int64Ty);
  
  //first level index
  llvm::Value *tmp = Builder.CreateLShr(thisPtr, 26);
  llvm::Value *level1Idx = Builder.CreateAnd(tmp, 4194303);
  
  //second level index
  tmp = Builder.CreateLShr(thisPtr, 3);
  llvm::Value *level2Idx = Builder.CreateAnd(tmp, 8388607);

  //Load the metadata table, and our entry
  //Address cfixxLookupPtr = Address(cfixxLookupGlobal, getPointerAlign());
  //llvm::Value *cfixxLookup = Builder.CreateLoad(cfixxLookupPtr);
  tmp = Builder.CreateGEP(cfixxLookupPtr, level1Idx);
  //tmp = Builder.CreateBitCast(tmp, CGM.VoidPtrTy->getPointerTo()->getPointerTo());
  Address secondLevelPtr = Address(tmp, getPointerAlign());
  llvm::Value *secondLevel = Builder.CreateLoad(secondLevelPtr);
  tmp = Builder.CreateGEP(secondLevel, level2Idx);
  Address entryPtr = Address(tmp, getPointerAlign());
  llvm::Instruction *VTable = Builder.CreateLoad(entryPtr);

  if(VTable->getType() != VTableTy)
    VTable = dyn_cast<llvm::Instruction>(Builder.CreateBitCast(VTable, VTableTy));
  /*End inlining - rest of this mirrors GetVtablePtr directly above*/
