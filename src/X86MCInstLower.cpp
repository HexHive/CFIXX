...
void X86AsmPrinter::EmitInstruction(const MachineInstr *MI) {
  ...
  /*NHB CFIXX*/
  /* not doing this anymore - directly adding mpx to things that mayWrite() later on
  if(MI->isCall() && MI->getOperand(0).isGlobal() && MI->getOperand(0).getGlobal()->getName() == "cfixxFakeMPXCheck") {
        auto bndChk = MCInstBuilder(X86::BNDCU64rm);
        bndChk.addReg(X86::BND0);
        bndChk.addReg(X86::RDI); //base
        bndChk.addImm(0); //scale
        bndChk.addReg(0); //index
        bndChk.addImm(0xff80fd6a);  //disp
        bndChk.addReg(0); //segment
        EmitAndCountInstruction(bndChk);
    return;
  }
  */
  if(MI->isCall() && MI->getOperand(0).isGlobal() && MI->getOperand(0).getGlobal()->getName() == "cfixxFakeMetadataWrite") {
    const MachineInstr *rsi, *rdi;
    rsi = rdi = nullptr;
    
    /*Track down rsi / rdi if possible */
    for (auto &tmp : *(MI->getParent())){
      const MachineOperand &MO = tmp.getOperand(0);
      if(&tmp == MI) break;
      else if(MO.isReg() && MO.getReg() == X86::RSI)
        rsi = &tmp;
      else if(MO.isReg() && MO.getReg() == X86::RDI)
        rdi = &tmp;
    }
    
    /*
    if(rsi) rsi->dump();
    else errs() << "RSI defined in another block\n";

    if(rdi) rdi->dump();
    else errs() << "RDI defined in another block\n";
    
    MI->dump();
    errs() << "\n";
    */
    unsigned opcode = 0;
    if(!rdi) opcode = X86::MOV64mr;
    else {
      switch(rdi->getOpcode()){
        case X86::MOV32ri:
        ...
        case X86::ADD64rm:
          ...
          break;
        default:
          assert(0 && "unknown way of finding the vtable pointer\n");
      }
    }
    auto OutMI = MCInstBuilder(opcode);
    if(!rsi){
      OutMI.addReg(X86::RSI);
      OutMI.addImm(0);
      OutMI.addReg(0);
      OutMI.addImm(0);
      OutMI.addReg(0);
      ...
      switch(rsi->getOpcode()){
        case X86::LEA64r: {
          for (int i = 1; i <= 5; ++i) {
            const MachineOperand &MO = rsi->getOperand(i);
            /*
             errs() << "\t";
            MO.print(errs());
            errs() << "\n";
            */
            if (auto MaybeMCOp = MCInstLowering.LowerMachineOperand(rsi, MO))
              OutMI.addOperand(MaybeMCOp.getValue());
            else{
              rsi->dump();
              errs() << "Operand: " << i << "\n";
              assert(0 && "Failed to handle a lea case when lowering metadata write\n");
            }
          }
          break;
        }
    ...
    */
    if(!rdi) {
      OutMI.addReg(X86::RDI); 
    }
    else {
      switch(rdi->getOpcode()){
        case X86::MOV32ri: 
        case X86::MOV64rr: {
          const MachineOperand &MO = rdi->getOperand(1);
          if (auto MaybeMCOp = MCInstLowering.LowerMachineOperand(rdi, MO))
            OutMI.addOperand(MaybeMCOp.getValue());
          else{
            rdi->dump();
            errs() << "Operand: 1\n";
            assert(0 && "Failed to deal with rdi when lower metadata write\n");
          }
          break;
        }
        case X86::LEA64r:
...
#define CASE_ALL_MOV_RM()               \
      ...
      case X86::LCMPXCHG64:
        {
          ...
          auto push = MCInstBuilder(X86::PUSH64r); 
          push.addReg(X86::RDI);
          EmitAndCountInstruction(push);
          
          auto lea = MCInstBuilder(X86::LEA64r);
          lea.addReg(X86::RDI);  //dest
          lea.addOperand(TmpInst.getOperand(0));  //base
          lea.addOperand(TmpInst.getOperand(1));  //scale
          lea.addOperand(TmpInst.getOperand(2));  //index
          lea.addOperand(TmpInst.getOperand(3));  //disp
          lea.addOperand(TmpInst.getOperand(4));  //seg
          EmitAndCountInstruction(lea);

          auto shr = MCInstBuilder(X86::SHR64ri);
          shr.addReg(X86::RDI);
          shr.addReg(X86::RDI);
          shr.addImm(24);
          EmitAndCountInstruction(shr);
          ...
          auto pop = MCInstBuilder(X86::POP64r);
          pop.addReg(X86::RDI);
          EmitAndCountInstruction(pop);
      ...
      case X86::MOV64toSDrm:
        {
          ...
          auto lea = MCInstBuilder(X86::LEA64r);
          lea.addReg(X86::RDI);  //dest
          lea.addOperand(TmpInst.getOperand(1));  //base
          lea.addOperand(TmpInst.getOperand(2));  //scale
          lea.addOperand(TmpInst.getOperand(3));  //index
          lea.addOperand(TmpInst.getOperand(4));  //disp
          lea.addOperand(TmpInst.getOperand(5));  //seg
          EmitAndCountInstruction(lea);
      ...
      case X86::XCHG64rm:  
        {
          ...
          auto lea = MCInstBuilder(X86::LEA64r);
          lea.addReg(X86::RDI);
          lea.addOperand(TmpInst.getOperand(2));  //base
          lea.addOperand(TmpInst.getOperand(3));  //scale
          lea.addOperand(TmpInst.getOperand(4));  //index
          lea.addOperand(TmpInst.getOperand(5));  //disp
          lea.addOperand(TmpInst.getOperand(6));  //seg
          EmitAndCountInstruction(lea);
  ...
  //TmpInst.dump();
  if(0 && MI->getOpcode() == X86::BNDCU64rr)
    MI->dump();
